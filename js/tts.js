...
import { stripMarkdown, splitIntoSentences } from './utils.js?v=1.3.15';
...
function countPronouncedChars(str) {
  ...
  for (var i = 0; i < str.length; i++) {
    ...
    if (c >= 0x4E00 && c <= 0x9FFF) { count++; continue; }
    if (c >= 0x3400 && c <= 0x4DBF) { count++; continue; }
    if (c >= 65 && c <= 90) { count++; continue; }
    if (c >= 97 && c <= 122) { count++; continue; }
    if (c >= 48 && c <= 57) { count++; continue; }
    if (c >= 0xFF10 && c <= 0xFF19) { count++; continue; }
    if (c >= 0xFF21 && c <= 0xFF3A) { count++; continue; }
    if (c >= 0xFF41 && c <= 0xFF5A) { count++; continue; }
  }
  return count;
}

/** 从 DOM 元素计算有效字数（优先使用 data-tts-chars） */
...
function startHighlightSyncLoop() {
  ...
  function syncHighlight() {
    ...
    if (segRanges && segRanges.length > 0 && currentIdx < segRanges.length) {
      ...
      var targetCharPos = charsBeforeSeg + effectiveProgress * curSegTotalChars;
...
function playNextSegment() {
  ...
  if (idx === 0) {
    var firstSentences = getTtsSentences(_currentBubbleEl);
    if (firstSentences.length > 0) {
      applyHighlight(firstSentences, 0);
    }
  }
...
export function speakViaWebSpeech(plainText, btnEl) {
  ...
  var wsBoundaryCalibrated = false;
  ...
  function wsSyncLoop() {
    ...
    var SPEAK_RATE_PER_CHAR_WS = 0.12;
    var estimatedSpeakingTime = wsTotalChars * SPEAK_RATE_PER_CHAR_WS;
    var ratio = 0;
    
    if (estimatedSpeakingTime > 0) {
      ratio = elapsed / estimatedSpeakingTime;
    } else {
      ratio = elapsed / wsEstimatedDuration;
    }
    
    if (wsBoundaryCalibrated) {
    ...
    if (wsFirstSentenceLock && domIdx > 0) {
      domIdx = 0;
    }
  ...
  utter.onstart = function () {
    ...
    if (wsSentences.length > 0) {
      ...
      wsLastIdx = 0;
      wsFirstSentenceLock = true;
      setTimeout(function () {
        wsFirstSentenceLock = false;
      }, 800);
      setTimeout(function () {
        wsRAF = requestAnimationFrame(wsSyncLoop);
      }, 300);
